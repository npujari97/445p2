...
def resize(X):
    ...
    for i in range(len(X)):
        ...
        img = img.resize((image_dim, image_dim), resample = Image.BICUBIC)
...
class ImageStandardizer(object):
    ...
    def fit(self, X):
        ...
        for i in range(0,3):
            ...
            std_arr.append(std)
        self.image_mean = mean_arr
        self.image_std = std_arr
    ...
    def transform(self, X):
        ...
        for image in range(X.shape[0]):
            for i in range(0,3):
                X[image,:,:,i] = (X[image,:,:,i] - self.image_mean[i])/self.image_std[i]
        return X
